...
const onTimeChange = (_event: any, selectedTime?: Date) => {
  ...
  if (selectedTime && activeTimeIndex !== null) {
    ...
    // Logic: 
    // 0-30 minutes -> Keep current hour (e.g., 6:30 becomes 6)
    // 31-59 minutes -> Move to next hour (e.g., 6:31 becomes 7)
    const formatted = m > 30 ? `${h + 1}` : `${h}`;
...
<ScrollView 
  ...
  onScroll={() => setDropdownVisible(false)} // Hide dropdown when user scrolls the form
  onTouchStart={() => setDropdownVisible(false)} // Hide when user touches background
>
  ...
  {/* Simplified: We just create a date with the whole number hour */}
  {new Date(`1970-01-01T${entry.hour.padStart(2, '0')}:00`)
        ...
        <Modal visible={isScanning} transparent={true} animationType="fade">
          <View style={styles.loadingOverlay}>
              <View style={styles.loadingBox}>
                  ...
                  <Text style={styles.loadingSubtitle}>Gemini AI is Reading Your Image... h</Text>
              </View>
          </View>
        </Modal>
